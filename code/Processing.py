...
def plant_classify(input_tif=config.index_total_file_cut,input_shp=config.border_shp,output_directory=config.result_floder):
    ...
    with open(output_txt, 'w') as txt_file:
        ...
        txt_file.write(f'MNDWI Threshold: {thresholds[0]}\n')
        txt_file.write(f'TC2 Threshold: {thresholds[1]}\n')
        txt_file.write(f'SVSI Threshold: {thresholds[3]}\n')
        txt_file.write(f'EVSI Threshold: {thresholds[2]}\n')
    ...
    for class_value in [1, 2, 3, 4, 5]:
        ...
        # 形态学开操作
        opened = binary_opening(mask, structure=disk(5))
